...
vector<DistancePair> sortedPairsByDistance(const vector<Point>& points){
    ...
    sort(pairs.begin(), pairs.end(),[](auto& a, auto& b){ return a.distance < b.distance; });
...
long long part2(const std::vector<Point>& points){
    ...
    auto distPairs = sortedPairsByDistance(points);

    map<Point, SharedCircuit> circuitsByPoint;
    pair<Point, Point> lastPair{};

    for (const auto& p : distPairs) {
        auto [_, circuitChg] = connect(p.a, p.b, circuitsByPoint);
...
int main(){
    ...
    cout << "Solution: " <<  part2(points) << endl;
